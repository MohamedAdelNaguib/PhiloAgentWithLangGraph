...
class PhilosopherExtract(BaseModel):
    ...
    @classmethod
    def from_json(cls: object, metadata_file: Path) -> list["PhilosopherExtract"]:
        """A function to create philosophers from a given json metadata
        with information about each one of them
        
        Args:
        cls: Philosopher class
        metadata_file: path to json file with list of metadata for different
        philosophers
